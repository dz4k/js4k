...
const actuate = (el, mkReq) => {
  ...
  const swapStyle = rel.slice("swap-".length) || "replaceChildren"
  fetch(mkReq(el)).then(res => res.text()).then(innerHTML =>
    Object.assign(document.createElement("template"), { innerHTML }).content
  ).then(content => target?.[swapStyle](content))
  return false
}
addEventListener("click", e => actuate(e.target.closest("a"), el => el.href))
addEventListener("submit", e => actuate(e.target, el => new Request(el.action, {
