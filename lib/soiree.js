...
const actuate = (e, el, mkReq) => {
  if (!el) return
  const rel = [...el.relList].find(r => /^swap/.test(r)); if (!rel) return
  e.preventDefault()
  const s = el.dataset.target ?? el.target
  const target = s ? el.getRootNode().querySelector(s) : el
  const swapType = rel.slice("swap-".length) || "replaceChildren"
  const req = mkReq(el); req.headers.set("soiree", "1")
  fetch(req).then(res => res.text()).then(c => target?.[swapType](
...
addEventListener("submit", e => actuate(e, e.target, el => new Request(
  el.action,
  { method: el.getAttribute("method") || "GET", body: new FormData(el) }
)))
